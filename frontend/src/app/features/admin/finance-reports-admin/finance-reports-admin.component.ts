...
const CHARGE_CATEGORY_LABELS: Record<string, string> = {
  ...
  ballBoyFee: 'Ball boy',
  guestFee: 'Guest fees',
  rentalRacketFee: 'Racket rental',
  rentalBalls50Fee: 'Balls (50-pack) rental',
  rentalBalls100Fee: 'Balls (100-pack) rental',
  rentalBallMachineFee: 'Ball machine rental',
...
export class FinanceReportsAdminComponent implements OnInit {
  ...
  async exportExcel() {
    ...
    summary.addRow(['Finance Report', this.selectedClub()?.clubName ?? '', this.rangeLabel()]).font = { bold: true, size: 13 };
    summary.addRow([]);
    ...
    a.download = `finance-report-${clubSlug}-${label}.xlsx`;
    a.click();
