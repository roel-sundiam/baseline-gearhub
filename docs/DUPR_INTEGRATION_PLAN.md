...
>
> **2026-07-29 update: a production-key review call with DUPR PM Haresh Chandiramani
> surfaced a real blocker** — DUPR's Platform Requirements make Premium Event support
> (`PREMIUM_L1` gating on event registration) mandatory, not optional as this doc
> previously assumed. Built and shipped for Hosted Play (see the User Gating entry
...
4. **Remaining open questions for `tech@mydupr.com` before a production-key submission:**
   ...
   - ~~Whether `PREMIUM_L1`/`VERIFIED_L1` gating needs anything beyond the `BASIC_L1` check already built~~ **Resolved for `PREMIUM_L1`** — confirmed live on the 2026-07-29 review call that Premium Event support is mandatory; built (see User Gating entry). `VERIFIED_L1` still unaddressed — no CourtGo resource needs identity verification today, worth confirming that's acceptable.
